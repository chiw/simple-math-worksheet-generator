<script lang="ts">
    ...
    import { 
        FunMultiplicationQuestionFormat, FUN_MULTIPLICATION_QUESTION_FORMAT, 
        FunMultiplicationWorksheetSize, FUN_MULTIPLICATION_WORKSHEET_SIZE
    ...
    $: {
        ...
        if(selectedWorksheetSize === FunMultiplicationWorksheetSize.A4) {
            console.log("set stores with A4 default config");
            questionConfigStore.set(AppConstants.FUN_MULTIPLICATION_WORKSHEET_DEFAULT_CONFIG.A4.questionConfig);
            horizontalMethodStyleConfigStore.set(AppConstants.FUN_MULTIPLICATION_WORKSHEET_DEFAULT_CONFIG.A4.horizonalMethodStyleConfig);
            columnMethodStyleConfigStore.set(AppConstants.FUN_MULTIPLICATION_WORKSHEET_DEFAULT_CONFIG.A4.columnMethodStyleConfig);
            worksheetConfigStore.set(AppConstants.FUN_MULTIPLICATION_WORKSHEET_DEFAULT_CONFIG.A4.worksheetConfig);
            worksheetContainerStyleConfigStore.set(AppConstants.FUN_MULTIPLICATION_WORKSHEET_DEFAULT_CONFIG.A4.worksheetCointainerStyleConfig);
        } else {
            console.log("set stores with A4_LANDSCAPE default config");
            questionConfigStore.set(AppConstants.FUN_MULTIPLICATION_WORKSHEET_DEFAULT_CONFIG.A4_LANDSCAPE.questionConfig);
            horizontalMethodStyleConfigStore.set(AppConstants.FUN_MULTIPLICATION_WORKSHEET_DEFAULT_CONFIG.A4_LANDSCAPE.horizonalMethodStyleConfig);
            columnMethodStyleConfigStore.set(AppConstants.FUN_MULTIPLICATION_WORKSHEET_DEFAULT_CONFIG.A4_LANDSCAPE.columnMethodStyleConfig);
            worksheetConfigStore.set(AppConstants.FUN_MULTIPLICATION_WORKSHEET_DEFAULT_CONFIG.A4_LANDSCAPE.worksheetConfig);
            worksheetContainerStyleConfigStore.set(AppConstants.FUN_MULTIPLICATION_WORKSHEET_DEFAULT_CONFIG.A4_LANDSCAPE.worksheetCointainerStyleConfig);
        }
...
<div style="margin-left:0.5em" class="nonPrintable">
    ...
    <div style="margin-left:0.5em">
        <details class="mdc-typography--subtitle1" style="padding-top:0.5em" bind:open={basicSettingsOpen}>
            ...
            <!-- <Set chips={FUN_MULTIPLICATION_WORKSHEET_SIZE} let:chip choice bind:selected={selectedWorksheetSize}>
                <Chip {chip}>
            ...
            <!-- <Select bind:value={$questionConfigStore.questionFormat} label="Question format">
                <Option value="" />
                {#each FUN_MULTIPLICATION_QUESTION_FORMAT as choice}
            ...
            <FormField>
                ...
                <span slot="label">{m.questionconfigshowfirstnumber()}</span>
            </FormField>

            <FormField>
                <Switch bind:checked={$questionConfigStore.showSecondNum} />
                <span slot="label">{m.questionconfigshowsecondnumber()}</span>
            </FormField>
        ...
        <details class="mdc-typography--subtitle1" style="padding-top:0.5em" bind:open={layoutSettingsOpen}>
            ...
            {#if $questionConfigStore.questionFormat === FunMultiplicationQuestionFormat.HORIZONTAL_METHOD}
                <div>
            ...
            {:else if $questionConfigStore.questionFormat === FunMultiplicationQuestionFormat.COLUMN_METHOD}
                <div>                    
